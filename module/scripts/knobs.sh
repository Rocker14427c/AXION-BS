...
meta_home_swap() {
  echo "Display|Black 6-app home|Swaps the launcher for the SPSM home screen while the mode is on, and restores your launcher on exit. If the new home does not come up, your launcher is put back within seconds.|1|session|core"
}
snapshot_home_swap() {
  printf 'home\t%s\n' "$(home_holder)"
...
meta_brightness_cap() {
  echo "Display|Dim the screen|Sets a low fixed backlight so SPSM looks like realme's dark mode. Set once and left alone - it will not fight your brightness slider. Only ever dims: if you keep the screen darker than this yourself, yours is kept.|1|session|battery"
}
...
probe_freeze_google() {
  ...
  for _p in $GOOGLE_PKGS; do
    has dumpsys || break
    _s=$(dumpsys package "$_p" 2>/dev/null | sed -n 's/^ *suspended=\([a-z]*\).*/\1/p' | head -1)
    [ -n "$_s" ] || continue
    printf 'suspended:%s\t%s\n' "$_p" "$_s"
    _any=$((_any + 1))
  done
  [ "$_any" = "0" ] && printf 'suspended:none\tno google package on this phone\n'
  return 0
}
...
meta_block_other_apps() {
  echo "Apps|Block other apps|Apps you have not allowed cannot be opened or run while the mode is on: they are suspended (their icons go grey) and stopped, and they come back to normal when you exit. The dialer, messages, alarms, your keyboard, your launcher and your root manager are never touched. Turn this off if you want the mode to leave other apps launchable.|1|session|battery,breaks-features"
}
...
meta_data_off() {
  echo "Network|Mobile data off|Turns mobile data off while the mode is on. On a weak signal the radio is one of the biggest drains, and the mode is meant for times when you are not using the phone - it is off by default because only you know whether a call or message has to get through.|0|session|battery,breaks-features"
}
...
meta_cap_always() {
  echo "Performance|Keep CPU and GPU limits while the screen is on|SPSM normally applies the CPU/GPU ceilings and the offline cores only while the screen is off, so the phone is never slow while you are using it - which is also why a kernel manager opened with the screen on shows everything normal. Turn this on to keep them in place the whole time the mode is on. Slower and cooler, and you can watch the limits in any kernel manager while the screen is on.|0|session|perf,breaks-features,control"
}
snapshot_cap_always() { :; }
apply_cap_always() { :; }
restore_cap_always() { :; }
