...
snapshot_cpu_cap() {
  ...
  # Recorded as the token, not the sentence. (MISSING) when the node does not
  # read as something we know: then it is not ours to change, here or on exit.
  _pm=$(pwr_mode_now)
  [ -n "$_pm" ] || _pm='(MISSING)'
  printf '%s\t%s\n' "$PWRMODE" "$(enc_val "$_pm")"
}
apply_cpu_cap() {
  ...
  # The MediaTek power mode: the biggest in-use lever this phone has, and now
  # safely reversible - written only when the node reads as a state we can put
  # back (see pwr_mode_token). A state we do not recognise is left alone and
  # said so, rather than written with a number nothing can undo.
  if [ -e "$(rp "$PWRMODE")" ]; then
    if [ -n "$(pwr_mode_now)" ]; then
      w 1 "$PWRMODE"
    else
      log "skip $PWRMODE: it does not read as a state we can put back"
    fi
  fi
}
