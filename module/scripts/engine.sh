...
do_probe() {
  lock_acquire || { echo "probe: busy"; return 1; }
  if [ -f "$ACTIVE" ]; then
    lock_release
    echo "probe: switch SPSM off first - it writes its own journal while it checks"
    return 1
  fi
  sync_scripts
  ...
  # with a session's.
  _J=$JOURNAL; _O=$ORIG_DIR; _S=$STATE; _L=$LOG
  JOURNAL="$PROBE_DIR/journal"; ORIG_DIR="$PROBE_DIR/journal/orig"; STATE="$PROBE_DIR/state"
  LOG="$PROBE_DIR/probe.log"
  : > "$LOG"

  log "probe: starting (module $(spsm_version))"
  _n=0; _works=0; _inert=0; _partial=0; _unknown=0
  for _k in $(knobs_all); do
    _v=$(probe_one "$_k")
    _verdict=${_v%%	*}
    _detail=${_v#*	}
  ...
  log "probe: done - $_works work, $_inert inert, $_partial partial, $_unknown unknown, of $_n"
  echo "works=$_works inert=$_inert partial=$_partial unknown=$_unknown total=$_n"
  ...
  JOURNAL="$_J"; ORIG_DIR="$_O"; STATE="$_S"; LOG="$_L"
  rm -rf "$PROBE_DIR"
  lock_release
  return 0
}
