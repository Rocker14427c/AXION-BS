...
knob_apply() { # knob_apply id
  ...
  # A knob counts as applied even if the apply function reported trouble: a
  # partial change is precisely the case where the revert MUST still run.
  j_record_state "$_id" applied
  j_order_add "$_id"

  if [ "$(norm "$_before")" = "$(norm "$_now")" ]; then
...
do_activate() {
  ...
  if [ -f "$ACTIVE" ]; then
    log "activate: already on, re-applying"
  else
    j_reset
    log "===== SPSM v3 ON ====="
  fi
  ...
  start_daemon
  progress "On"
  log "SPSM ON: $(applied_count) knobs applied"
  lock_release
  return 0
}
...
do_deactivate() {
  ...
  if [ "${DRIFT:-0}" = "0" ]; then
    ...
    log "revert finished with $DRIFT drifted knob(s) - see journal"
