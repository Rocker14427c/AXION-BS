...
run_engine activate >/dev/null 2>&1
[ "$(cat "$ROOT/sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq")" = "1800000" ]
check "by default nothing is capped while the screen is on" $?
run_engine deactivate >/dev/null 2>&1
make_tree; make_stubs; seed_stub_state
enable_knobs cpu_cap gpu_cap cpu_offline_big deep_doze
...
say "59. the phone cannot be left in the CPU's Low Power mode after an exit"
# From the phone's own log, v3.0.12:
#   WARN cpu_cap did not return: cpufreq_power_mode:
#        want [Default(Normal) mode] got [Low Power mode]
# The node is written with a number and answers with a sentence. Recording the
# sentence meant the exit wrote words into a node that only takes a digit, so the
# phone stayed in Low Power mode and the exit then ran its safety pass for 59s.
make_tree; make_stubs; seed_stub_state
enable_knobs cpu_cap
screen_on
F="$ROOT/proc/cpufreq/cpufreq_power_mode"
printf 'Default(Normal) mode\n' > "$F"      # exactly what the phone answers
[ "$(cat "$F")" = "Default(Normal) mode" ]
check "the phone starts in its normal power mode" $?
run_engine activate >/dev/null 2>&1
screen_off
run_engine screen-off >/dev/null 2>&1
[ "$(cat "$F")" = "1" ]
check "the cap puts it into Low Power mode while the screen is off" $?
[ "$(awk -F'\t' '$1 ~ /cpufreq_power_mode$/ {print $2}' "$WORK/spsm/journal/cpu_cap.orig")" = "0\\n" ]
check "the journal holds the number, not the sentence ($(grep cpufreq_power_mode "$WORK/spsm/journal/cpu_cap.orig"))" $?
screen_on
run_engine screen-on >/dev/null 2>&1
[ "$(cat "$F")" = "0" ]
check "waking puts it back to normal, and the write is accepted" $?
screen_on
run_engine deactivate > "$WORK/out.d59" 2>&1
[ "$(cat "$F")" = "0" ]
check "and the exit leaves it in the normal mode it found" $?
grep -q "did not return" "$WORK/spsm/spsm.log"
if [ $? = 0 ]; then bad "no false drift is reported for it"; else ok "no false drift is reported for it"; fi
grep -q "revert finished in .* with 0 drifted" "$WORK/spsm/spsm.log" || grep -q "revert clean" "$WORK/spsm/spsm.log"
check "so the exit is clean and quick, with no safety pass" $?

# A state we do not recognise is not ours to change: the node is left alone
# rather than written with a number nobody can undo.
make_tree; make_stubs; seed_stub_state
enable_knobs cpu_cap
